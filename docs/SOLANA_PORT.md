...
| Walrus proof storage | Unchanged — Walrus is chain-agnostic; only the on-chain hash anchor moves to the Solana program. `finish_race` persists `race.proof_hash`; `settle_market` persists `market.winning_proof_hash` + `market.walrus_blob_id` (+ `settled_at`) as queryable account state, not just an emitted log. | ✅ anchor fields landed (race + market) |
